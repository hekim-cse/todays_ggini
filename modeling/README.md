...
<p align="center">
  ...
  RAG 후보 메뉴 평가부터 OR-Tools 기반 월간 대표 식단 최적화,
  MMR 기반 대안 메뉴 구성 및 품질 검증까지 수행합니다.
</p>
...
6. [OR-Tools 월간 식단 최적화](#6-or-tools-월간-식단-최적화)
7. [MMR 기반 대안 메뉴 재랭킹](#7-mmr-기반-대안-메뉴-재랭킹)
8. [월간 식단 결과 매핑](#8-월간-식단-결과-매핑)
...
이후 식단 추천 단계에서는 Backend로부터 전달받은 예산, 식사 횟수, 식단 목표, 조리 수준, 선호도, 알레르기 등의 조건을 추천 프로필로 변환합니다. RAG가 제공한 후보 메뉴를 평가한 뒤, OR-Tools로 월간 대표 메뉴를 최적화하고 MMR을 통해 각 대표 메뉴의 대안 메뉴를 다양화합니다.
...
<table style="background-color:#EAF4FF; border-left:6px solid #4D96D9; padding:12px; width:100%;">
  <tr>
    <td>
      ...
      OR-Tools가 대표 식단을 최적화합니다.
      이후 MMR은 대표 메뉴를 유지한 상태에서 대안 메뉴의 유사도와 노출 횟수를 조정합니다.
    </td>
  </tr>
</table>
...
OR-Tools CP-SAT Monthly Plan Optimizer
  └── 각 식사 슬롯의 selected_menu 확정
          ↓
MMR Alternative Re-ranking
  └── selected_menu별 alternative_menus 구성
          ↓
Monthly Plan Result Mapping
          ↓
Plan Summary
          ↓
...
| Optimizer Candidate Builder | `final_score` 상위 후보와 예산 충족에 필요한 저비용 후보를 병합 |
| Optimizer | OR-Tools를 사용해 월 전체 식사 슬롯의 `selected_menu` 최적화 |
| MMR Alternative Re-ranking | 대표 메뉴를 유지하면서 유사도와 노출 횟수를 고려해 `alternative_menus` 구성 |
...
## 6. OR-Tools 월간 식단 최적화

월간 식단의 대표 메뉴는 OR-Tools CP-SAT Solver를 사용하여 구성합니다.

단순히 점수가 높은 메뉴를 순서대로 배치하는 방식이 아니라, 전체 식단 기간을 하나의 조합 최적화 문제로 정의합니다.

### Optimizer 후보 구성

OR-Tools에는 전체 RAG 후보를 그대로 전달하지 않습니다.

기본적으로 `final_score`가 높은 후보를 우선 사용하되, 점수 상위 후보만으로 월간 예산을 맞추기 어려워지는 문제를 방지하기 위해 저비용 후보를 추가로 병합합니다.

```text
전체 추천 후보
→ Final Score 상위 후보 추출
→ 저비용 후보 추가 추출
→ 메뉴 ID 기준 중복 제거
→ OR-Tools 후보 집합 구성
```

MMR 점수는 OR-Tools 입력 후보를 구성하거나 대표 메뉴를 결정하는 데 사용하지 않습니다.
...
- 식단 목표와 영양 조건 보정
- 조리 난이도와 사용자 조건 반영
- 제한시간 안에서 유효한 월간 식단 생성

### 최적화 처리 흐름

```text
Final Score 상위 후보 + 저비용 후보
→ Optimizer 입력 데이터 생성
→ 후보·예산·반복 정책 구성
→ 의사결정 변수 및 제약조건 생성
→ 목적함수 구성
...
| 상태 | 처리 |
|---|---|
| `OPTIMAL` | 제약조건을 만족하는 최적해 |
| `FEASIBLE` | 제약조건을 만족하지만 최적성이 증명되지 않은 유효해 |
| `INFEASIBLE` | 현재 후보와 제약조건으로 구성 가능한 식단이 없음 |
| `UNKNOWN` | 제한시간 또는 자원 문제로 결과를 판단하지 못함 |

`OPTIMAL`과 `FEASIBLE`은 월간 대표 식단으로 사용할 수 있으며, 결과는 이후 Validator에서 다시 검증합니다.
...
- 후보 메뉴 수와 반복 수용량 확인
- 필요 끼니 대비 후보 비율 확인
- 최소 비용 기준 예산 충족 가능성 진단
- 추가 저비용 후보 확보
- 추가 RAG 후보 요청
- 완화 가능한 정책 조정 후 재실행
- 최종 실패 사유를 구조화된 응답으로 반환
...
MMR은 OR-Tools가 확정한 월간 대표 메뉴를 다시 선택하는 데 사용하지 않습니다.

OR-Tools가 각 식사 슬롯의 `selected_menu`를 확정한 뒤, 해당 대표 메뉴를 유지하면서 사용자에게 제공할 `alternative_menus`를 구성하는 후처리 단계에 적용합니다.

```text
OR-Tools selected_menu 확정
→ 동일 메뉴 제외
→ 대표 메뉴 및 기존 노출 메뉴와의 유사도 계산
→ Final Score와 유사도 패널티 결합
→ 기존 사용 횟수 패널티 적용
→ MMR 점수 기준 대안 메뉴 선택
```
...
- 대표 메뉴와 지나치게 유사한 대안 제거
- 이전 식사에서 이미 노출된 메뉴의 반복 완화
- 같은 끼니의 대안 메뉴끼리 유사하게 구성되는 문제 방지
- 사용자 다양성 수준 반영
- 대표 메뉴를 변경하지 않으면서 선택 가능한 대안 제공

### 비교 대상

MMR은 대안 후보를 다음 메뉴들과 비교합니다.

1. 현재 끼니의 `selected_menu`
2. 이전에 노출된 `selected_menu`
3. 이전에 노출된 `alternative_menus`
4. 현재 끼니에서 이미 선택된 다른 대안 메뉴
...
사용자가 다양성을 강하게 설정할수록 추천 관련성보다 유사도 감점의 비중을 높입니다.
...
OR-Tools가 선택한 대표 메뉴와 MMR이 선택한 대안 메뉴를 Backend와 Frontend에서 사용할 수 있는 월간 식단 응답 구조로 변환합니다.
...
- OR-Tools가 확정한 `selected_menu`는 변경하지 않습니다.
- MMR은 각 대표 메뉴의 `alternative_menus`만 구성합니다.
- 대안 메뉴는 대표 메뉴와 동일하거나 지나치게 유사한 후보를 우선 제외합니다.
- 월간 비용·칼로리·중복률 등의 Summary는 대표 메뉴를 기준으로 계산합니다.
- 대안 메뉴는 사용자에게 교체 가능한 선택지로 별도 제공됩니다.

관련 모듈:

```text
services/optimizer/ortools/
└── result_mapper.py
...
| Plan | [`services/plan/README.md`](services/plan/README.md) | OR-Tools 결과 매핑, MMR 대안 메뉴 구성, 식단 요약 및 품질 검증 |
...
- RAG 응답 매핑 및 품질 진단
- 후보 메뉴 점수 계산
- Final Score 상위 후보와 저비용 Optimizer 후보 구성
- OR-Tools 기반 월간 대표 메뉴 최적화
- MMR 기반 대안 메뉴 재랭킹 및 다양성 제어
- 월간 식단 결과 매핑
