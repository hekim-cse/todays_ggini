...
Recommendation이 점수화한 후보 메뉴를 대상으로 OR-Tools가 월간 식단의 대표 메뉴를 최적화하고, 이후 MMR 기반 후처리로 각 대표 메뉴의 대안 메뉴를 구성하는 CP-SAT 기반 최적화 모듈입니다.
...
| 단계 | 역할 |
|---|---|
| Recommendation | 후보 메뉴별 사용자 적합도와 `final_score` 계산 |
| Optimizer Candidate Builder | Final Score 상위 후보와 예산 충족에 필요한 저비용 후보 병합 |
| OR-Tools Optimizer | 전체 월간 Slot의 `selected_menu` 조합 최적화 |
| MMR Alternative Re-ranking | OR-Tools가 확정한 대표 메뉴를 유지하면서 `alternative_menus` 다양화 |
| Plan Mapper | 대표 메뉴와 대안 메뉴를 일자별 월간 응답 구조로 변환 |
| Validation | 생성된 월간 식단의 품질과 Style 반영 여부 검증 |
...
| 파일 | 역할 |
|---|---|
| `optimizer_input_builder.py` | Final Score 상위 후보와 저비용 후보를 병합하고 OR-Tools 입력과 설정을 구성 |
| `monthly_plan_optimizer.py` | CP-SAT 모델, 제약조건, Objective와 Solver 실행 |
| `result_mapper.py` | OR-Tools의 `selected_menu`를 유지하고 MMR 대안 메뉴를 추가해 월간 식단 구조로 매핑 |
...
| `meal_selector_service.py` | OR-Tools가 확정한 대표 메뉴별 MMR 대안 메뉴 선택 |
| `menu_diversity_service.py` | 기존 노출 메뉴와 사용 횟수를 반영한 다양성 제어 |
...
Solver의 `selected_items`에 포함된 `selected_menu`는 OR-Tools가 이미 확정한 대표 메뉴입니다.

`build_ortools_monthly_plan()`은 이 대표 메뉴를 다시 선택하거나 변경하지 않으며, 각 대표 메뉴에 대해 MMR 기반 대안 메뉴를 추가한 뒤 날짜·끼니별 Plan 구조로 매핑합니다.
...
OR-Tools는 각 식사 슬롯의 대표 메뉴인 `selected_menu`를 확정합니다.

Result Mapper는 OR-Tools가 확정한 대표 메뉴를 변경하지 않고, 전체 Recommendation 후보를 대상으로 MMR을 적용하여 각 끼니의 `alternative_menus`를 구성합니다.

현재 구현에서는 각 식사 슬롯에 최대 두 개의 대체 메뉴를 제공합니다.

### 처리 흐름

```text
OR-Tools selected_menu 확정
→ selected_menu와 동일한 후보 제외
→ 대표 메뉴 및 기존 노출 메뉴와의 유사도 계산
→ Final Score 기반 관련성 반영
→ 메뉴 유사도 패널티 반영
→ 기존 사용 횟수 패널티 반영
→ MMR 점수 기준 대안 메뉴 선택
→ selected_menu와 alternative_menus를 월간 Plan에 매핑
```

### 대체 메뉴 생성 원칙

- OR-Tools가 확정한 `selected_menu`는 변경하지 않습니다.
- MMR 점수는 OR-Tools 후보 구성이나 Objective 계산에 사용하지 않습니다.
- 대표 메뉴와 동일한 `menu_id`를 가진 후보는 제외합니다.
- 대표 메뉴와 지나치게 유사한 후보는 우선 제외합니다.
- 이전에 노출된 대표 메뉴와 대체 메뉴도 유사도 및 사용 횟수 계산에 반영합니다.
- 같은 끼니에 선택된 대체 메뉴끼리도 서로 유사하지 않도록 검사합니다.
- 조건을 만족하는 후보가 부족하면 두 개보다 적은 대체 메뉴가 반환될 수 있습니다.
- 사용할 수 있는 대체 후보가 없으면 빈 배열을 반환합니다.

### MMR 비교 대상

대체 메뉴 후보는 다음 메뉴들과 비교합니다.

1. 현재 식사 슬롯의 `selected_menu`
2. 이전에 노출된 `selected_menu`
3. 이전에 노출된 `alternative_menus`
4. 현재 끼니에서 이미 선택된 다른 대체 메뉴
...
MMR 기반 대체 메뉴 최대 두 개 생성
대체 메뉴 ID 중복 방지
대표 메뉴 및 기존 노출 메뉴와의 유사도 완화
같은 끼니의 대체 메뉴 간 유사도 완화
대체 후보가 없으면 빈 배열 반환
```

이 테스트는 OR-Tools가 확정한 대표 메뉴를 변경하지 않고, Result Mapper가 MMR 기반 대체 메뉴를 올바르게 구성하는지를 확인합니다.
...
### MMR은 OR-Tools 대표 메뉴를 변경하지 않음

OR-Tools 기반 월간 식단 생성 경로에서는 Solver가 각 식사 슬롯의 대표 메뉴를 먼저 확정합니다.
...
따라서 MMR 점수는 OR-Tools 입력 후보를 선별하거나 Solver Objective를 계산하는 데 사용되지 않습니다.

비-OR-Tools 기간별 Plan 생성 경로에서는 MMR이 대표 메뉴 선택에도 사용될 수 있지만, 현재 OR-Tools 기반 월간 최적화 경로와는 별도의 처리 흐름입니다.

### 대체 메뉴는 생성 시점 Summary에 포함되지 않음

월간 총비용, 칼로리, 영양 성분과 메뉴 중복률 등의 Summary는 `selected_menu`만을 기준으로 계산합니다.
...
따라서 사용자가 실제로 대체 메뉴를 선택하면 비용과 영양 수치가 최초 식단 생성 시점의 Summary와 달라질 수 있습니다.

운영 단계에서는 메뉴 교체 시 남은 예산과 영양 조건을 다시 검증하는 로직을 적용할 수 있습니다.
