...
Recommendation이 생성한 `final_score`는 월간 OR-Tools 경로에서는 Optimizer 후보 구성과 Objective의 추천 적합도 값으로 사용되고, 비-OR-Tools 경로에서는 MMR과 Style Priority를 통한 대표 메뉴 선택에 사용됩니다.
...
<table style="background-color:#EAF4FF; border-left:6px solid #4D96D9; padding:12px; width:100%;">
  <tr>
    <td>
      ...
      Recommendation은 월간 식단 전체 조합이나 최종 대표 메뉴를 직접 확정하지 않습니다.
      각 후보 메뉴가 현재 사용자에게 얼마나 적합한지를 계산하고,
      이후 단계에서 사용할 <code>final_score</code>와 설명 정보를 생성합니다.<br><br>
      월간 OR-Tools 경로에서는 이 점수를 바탕으로 Optimizer 후보를 구성하고
      OR-Tools가 <code>selected_menu</code>를 확정합니다.
      이후 MMR은 확정된 대표 메뉴를 유지하면서 <code>alternative_menus</code>를 구성합니다.<br><br>
      비-OR-Tools 경로에서는 MMR과 Style Priority가 Recommendation 결과를 사용해
      대표 메뉴와 대체 메뉴를 선택합니다.
    </td>
  </tr>
</table>
...
Recommendation의 다양성 점수는 순차 추천 과정에서 이미 선택된 메뉴와의 유사성을 반영하는 내부 점수입니다.

이 점수는 Plan 단계의 MMR과 별개이며, MMR은 Recommendation이 계산한 `final_score`에 최근 노출 메뉴 유사도와 사용 횟수를 추가로 반영합니다.
...
비-OR-Tools 경로
→ MMR의 Relevance Score로 사용
→ Style Priority와 함께 selected_menu 선택
...
이 단계의 순차 선택 결과는 Recommendation 후보의 점수와 순서를 생성하기 위한 처리입니다.

월간 OR-Tools 경로에서 이 순서가 식사 슬롯별 최종 `selected_menu`를 직접 확정하는 것은 아닙니다. 이후 Optimizer Candidate Builder와 OR-Tools Solver가 전체 월간 조합을 별도로 결정합니다.

<br>

## 21. MMR 및 Optimizer와의 역할 구분

Recommendation, MMR과 OR-Tools Optimizer는 모두 메뉴 선택 과정에 관여하지만, 실행 경로와 책임이 다릅니다.
...
Recommendation의 결과는 특정 식사 슬롯의 대표 메뉴 확정값이 아니라, 이후 단계가 사용할 후보별 적합도와 설명 정보입니다.
...
이 경로에서 OR-Tools가 월간 식사 슬롯별 대표 메뉴를 확정합니다.

MMR은 OR-Tools가 확정한 `selected_menu`를 다시 선택하거나 변경하지 않습니다.

또한 MMR 점수는 다음 처리에 사용되지 않습니다.

```text
Optimizer 후보 선별
OR-Tools 결정 변수 생성
OR-Tools Constraint
OR-Tools Objective 계산
selected_menu 확정
```
...
이 경로에서는 MMR과 Style Priority가 대표 메뉴 선택에도 사용됩니다.
...
MMR은 Recommendation이 생성한 `final_score`와 최근 노출 메뉴 유사도, 사용 횟수를 결합합니다.

```text
MMR Score
= final_score 기반 Relevance
- 최근 노출 메뉴 Similarity Penalty
- 메뉴 사용 횟수 Penalty
```

MMR의 역할은 실행 경로에 따라 다릅니다.

```text
월간 OR-Tools 경로
→ alternative_menus 구성에만 사용

비-OR-Tools 경로
→ selected_menu와 alternative_menus 구성에 사용
```

Recommendation의 `diversity_score`는 최종 점수의 구성 요소이고, MMR은 Plan 단계에서 수행하는 별도의 다양성 재랭킹입니다.
...
Optimizer는 개별 메뉴 하나의 적합도를 다시 계산하는 것이 아니라, 전체 기간의 대표 메뉴 조합을 결정합니다.
...
+ Protein Bonus
+ Difficulty Bonus
+ Nutrition Outlier Penalty 설정
→ 월간 식사 슬롯별 selected_menu 조합
```
...
<table style="background-color:#FFF6C7; border-left:6px solid #E6C85C; padding:12px; width:100%;">
  <tr>
    <td>
      ...
      Recommendation은 후보별 사용자 적합도인 <code>final_score</code>를 계산합니다.<br><br>
      월간 OR-Tools 경로에서는 Optimizer가 이 점수를 포함한 후보 정보를 바탕으로
      식사 슬롯별 <code>selected_menu</code>를 확정하고,
      MMR은 대표 메뉴를 유지하면서 <code>alternative_menus</code>를 구성합니다.<br><br>
      비-OR-Tools 경로에서는 MMR과 Style Priority가
      대표 메뉴와 대체 메뉴 선택에 모두 사용됩니다.
    </td>
  </tr>
</table>
...
| `optimizer_input_builder.py` | Final Score 상위 후보와 저비용 후보를 병합해 OR-Tools 입력 구성 |
| `monthly_plan_optimizer.py` | Recommendation 점수를 포함한 Objective로 월간 `selected_menu` 최적화 |
| `mmr_service.py` | Final Score, 최근 노출 메뉴 유사도와 사용 횟수를 결합한 MMR 재랭킹 |
...
### Recommendation 순차 선택과 월간 대표 메뉴 확정은 다름

`recommend_menus()` 내부에서는 높은 점수의 후보를 순차적으로 선택해 추천 결과 목록을 생성합니다.

하지만 월간 OR-Tools 경로에서 이 순차 선택 결과가 식사 슬롯별 최종 대표 메뉴를 의미하지는 않습니다.

```text
Recommendation 순차 선택
→ 점수화된 후보 목록과 순서 생성
...
따라서 Recommendation의 `selected_menu_ids`와 월간 Plan의 `selected_menu`를 같은 개념으로 해석하지 않도록 주의해야 합니다.

### MMR 역할은 실행 경로에 따라 다름
...
비-OR-Tools 경로
→ MMR과 Style Priority가 selected_menu 선택
→ MMR이 alternative_menus 구성
```

MMR이 항상 Recommendation 직후 실행되어 OR-Tools 입력 후보를 재정렬한다고 설명하면 실제 월간 경로와 다릅니다.

### Recommendation 다양성 점수와 MMR은 별도 단계

Recommendation의 `diversity_score`는 `similar_menu_ids`와 순차 선택 이력을 사용해 `final_score` 안에 반영됩니다.

Plan의 MMR은 다음 값을 별도로 사용합니다.
...
두 단계 모두 다양성을 다루지만 계산 시점과 역할이 다릅니다.
...
<table style="background-color:#FFF6C7; border-left:6px solid #E6C85C; padding:12px; width:100%;">
  <tr>
    <td>
      ...
      후보 부족을 줄이면서 사용자 맞춤 추천 품질을 유지합니다.<br><br>
      월간 OR-Tools 경로에서는 이 결과를 바탕으로 OR-Tools가 대표 메뉴를 확정하고,
      이후 MMR이 대표 메뉴를 유지하면서 대체 메뉴를 구성합니다.
    </td>
  </tr>
</table>