...
사용자가 입력한 식단 설정값을 RAG 후보 요청, Recommendation 점수 계산, Style 가중치 적용, OR-Tools 월간 대표 메뉴 최적화와 Plan 다양성 처리에서 공통으로 사용할 수 있는 Modeling Profile로 변환하는 모듈입니다.
...
<table style="background-color:#EAF4FF; border-left:6px solid #4D96D9; padding:12px; width:100%;">
  <tr>
    <td>
      ...
      공통 Profile로 변환하여 RAG, Recommendation, Style, Optimizer와 Plan 단계에 제공합니다.<br><br>
      월간 OR-Tools 경로에서는 Recommendation이 Profile을 사용해 후보별
      <code>final_score</code>를 계산하고, Optimizer가 식사 슬롯별
      <code>selected_menu</code>를 확정합니다.
...
이 값은 Recommendation에서 사용자 조리 수준 대비 메뉴 난이도 적합도를 계산하는 기준으로 사용됩니다.

월간 OR-Tools 경로에서는 Recommendation 결과에 포함된 `scores.difficulty`와 Optimizer 설정을 통해 간접적으로 대표 메뉴 선택에 반영되며, 최종 Plan의 평균 Difficulty Score는 Style Validation에도 사용됩니다.
...
### Recommendation 다양성과 Plan MMR의 차이

`diversity_penalty_strength`는 Recommendation의 순차 추천 과정에서 이미 선택된 유사 메뉴를 감점하는 데 사용됩니다.

```text
Recommendation
→ similar_menu_ids와 selected_menu_ids 비교
→ diversity_score 계산
→ final_score에 반영
```

Plan 단계의 MMR은 별도의 다양성 처리입니다.
...
월간 OR-Tools 경로에서는 MMR이 Optimizer 후보를 선별하거나 대표 메뉴를 다시 선택하는 데 사용되지 않습니다.
...
비-OR-Tools 경로에서는 MMR과 Style Priority가 대표 메뉴 선택에도 사용될 수 있습니다.
...
Profile은 월간 식단 Optimizer 입력을 구성하는 기초 조건을 제공합니다.
...
다만 Profile이 그대로 OR-Tools 메뉴 입력이 되는 것은 아닙니다.
...
Profile의 가중치와 사용자 조건은 Recommendation 점수 계산에 먼저 반영되고, 그 결과가 Optimizer의 후보 구성과 Objective에 전달됩니다.
...
monthly_budget
→ 월 예산 Hard Constraint

weights + meal_budget + meal_calorie_target
→ Recommendation final_score 계산

max_difficulty
→ Recommendation difficulty_score 계산 기준

goals와 선택 Style
→ Recommendation Soft Constraint 및 Optimizer Bonus 설정에 영향
```

`diversity_penalty_strength`는 Recommendation의 다양성 점수 계산에 사용되지만, MMR 점수 자체가 OR-Tools 후보 선별이나 Objective에 직접 사용되지는 않습니다.
...
Profile 자체는 Solver를 실행하거나 대표 메뉴를 선택하지 않습니다.
...
<table style="background-color:#FFF6C7; border-left:6px solid #E6C85C; padding:12px; width:100%;">
  <tr>
    <td>
      ...
      Profile은 추천과 최적화에 사용할 사용자 조건과 기준값을 생성합니다.
      Recommendation은 후보별 적합도 점수를 계산하고,
      Optimizer는 해당 점수와 월 예산·반복 제한을 이용해
      월간 <code>selected_menu</code>를 확정합니다.
      이후 Plan의 MMR은 대표 메뉴를 유지하면서 대체 메뉴를 구성합니다.
    </td>
  </tr>
</table>
...
### Profile 다양성 설정과 MMR을 혼동하지 않음

Profile의 `diversity_penalty_strength`는 Recommendation 내부의 `diversity_score` 계산에 사용됩니다.
...
Plan의 MMR은 Recommendation 이후 별도로 실행되며 다음 정보를 사용합니다.
...
월간 OR-Tools 경로에서는 MMR이 대표 메뉴를 다시 선택하지 않고 `alternative_menus` 구성에만 사용됩니다.

### Profile은 최종 메뉴 선택 결과를 포함하지 않음

Modeling Profile에는 다음 결과가 포함되지 않습니다.
...
따라서 Profile 응답은 사용자 조건을 정규화한 중간 데이터이며, 실제 추천 또는 월간 식단 결과로 해석하면 안 됩니다.
...
<table style="background-color:#FFF6C7; border-left:6px solid #E6C85C; padding:12px; width:100%;">
  <tr>
    <td>
      ...
      RAG·Recommendation·Style·Optimizer·Plan에서 공통으로 사용할
      한 끼 예산, 칼로리 기준, 목표별 가중치, 조리 난이도와 다양성 정책을 추가합니다.<br><br>
      Profile은 메뉴를 직접 선택하지 않으며,
      Recommendation이 후보별 적합도를 계산하고,
      OR-Tools가 월간 대표 메뉴를 확정한 뒤,
      Plan 단계의 MMR이 대체 메뉴를 구성할 수 있도록
      공통 사용자 조건을 제공하는 역할을 담당합니다.
    </td>
  </tr>
</table>